#!/usr/bin/env python3
"""Test the full implementation with identity filtering"""
...
def main():
    ...
    print("="*80)
    print("TESTING FULL IMPLEMENTATION")
    print("="*80)
    ...
    events = client.get_people_counter_full_coverage(
        from_time=from_time,
        to_time=to_time
    )
    ...
    if events:
        ...
        if times:
            ...
            for hour in sorted(hour_counts.keys()):
                print(f"  {hour}:00 - {hour_counts[hour]} events")
        ...
        if zero_count > 0:
            print(f"\nZero-count events: {zero_count}/{len(events)} ({zero_count/len(events)*100:.1f}%)")

        # Show location tags
        location_tags = set()
        for e in events:
            tags = e.get('_location_tags', [])
            if tags:
